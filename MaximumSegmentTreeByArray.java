public class MaximumSegmentTreeByArray extends SegmentTreeByArray {
     ...
     * Saves max value in each node (this.tree[treeIdx]).
     * 
    ...
    protected int rec_tree_build(int[] arr, int start, int end, int treeIdx) {
        ...
        this.tree[treeIdx] = Math.max(left,right);
        return this.tree[treeIdx]; 
    }

    /**
     * Helper function used to update the node based on the max value of its children
     * 
    ...
    protected void updateNode(int nodeIdx) {
        ...
        this.tree[nodeIdx] = Math.max(this.tree[nodeIdx*2 + 1], this.tree[nodeIdx*2 + 2]);
    }

    /**
     * Queries the Segment Tree for the maximum value.
    ...
    protected int query(int node, int start, int end, int left, int right) {
        ...
        if (right <= mid) {
            ...
            newNode = Math.max(leftNode,rightNode);
        }
