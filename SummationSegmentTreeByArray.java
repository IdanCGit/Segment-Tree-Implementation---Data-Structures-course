public class SummationSegmentTreeByArray extends SegmentTreeByArray {
     ...
     * Saves sum value in each node (this.tree[treeIdx]).
     * 
    ...
    protected int rec_tree_build(int[] arr, int start, int end, int treeIdx) {
        ...
        this.tree[treeIdx] = left + right;
        return this.tree[treeIdx]; 
    }

    /**
     * Helper function used to update the node based on the sum value of its children
     * 
    ...
    protected void updateNode(int nodeIdx) {
        ...
        this.tree[nodeIdx] = this.tree[nodeIdx*2 + 1] + this.tree[nodeIdx*2 + 2];
    }

    /**
     * Queries the Segment Tree for the summation value.
